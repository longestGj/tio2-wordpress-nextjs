...
export function buildMalaysiaRequestSampleMetadata(site:SiteConfig,options:{readonly indexingAuthorized:boolean;readonly seo?:Pick<typeof contract.seo,'title'|'description'>;readonly env?:Readonly<Record<string,string|undefined>>}):Metadata{
  ...
  return {title:(options.seo ?? contract.seo).title,description:(options.seo ?? contract.seo).description,alternates:{canonical:contract.seo.canonical},robots:{index:indexable,follow:indexable},openGraph:{type:'website',url:contract.seo.canonical,siteName:site.name,title:(options.seo ?? contract.seo).title,description:(options.seo ?? contract.seo).description,images:[]}}
}
