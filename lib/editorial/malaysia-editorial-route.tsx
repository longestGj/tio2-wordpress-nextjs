...
export async function renderMalaysiaEditorialRoute(pageId:string) {
  const {site,page}=await requestPage(pageId)
  const schema=buildEditorialJsonLd(site,page)
  return <MalaysiaEditorialPage page={page} structuredData={schema?<script type="application/ld+json" dangerouslySetInnerHTML={{__html:serializeJsonLd(schema)}}/>:undefined}/>
}
