...
REQUIRED_FILES = (
    ...
    "release_contract.py", "release_state.py", "release_actions.py", "release_baseline.py", "backup_core.py", "backup.sh", "sudoers.tio2-release", "sshd-tio2-production.conf",
)
...
def install_bootstrap(source: Path, paths: BootstrapPaths, *, deploy_uid: int, deploy_gid: int, stat_reader: Callable[[Path], os.stat_result] = os.lstat, self_test: Callable[[Path], None] = _default_self_test, sudo_validator: Callable[[Path], None] = _default_sudo_validator, fail_after: str | None = None, abrupt_after: str | None = None) -> None:
    ...
    try:
        for name in ("bootstrap_install.py", "bootstrap_selftest.py", "tio2_release.py", "release_contract.py", "release_state.py", "release_actions.py", "release_baseline.py", "backup_core.py", "backup.sh", "sshd-tio2-production.conf"):
