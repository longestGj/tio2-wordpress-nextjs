...
readonly BACKUP_ROOT=/opt/tio2-production/backups/releases
...
restart_wordpress() {
  ...
  compose up --wait wordpress >/dev/null
  compose run --rm --no-deps wpcli wp maintenance-mode deactivate --allow-root >/dev/null
}

cleanup() {
  status=$?
  if [ "$maintenance_armed" -eq 1 ] && [ "$deployment_continues" -eq 0 ]; then
    restart_wordpress || status=1
  fi
  if [ "$published" -eq 0 ]; then
    [ -z "$backup_dir" ] || /usr/bin/rm -rf -- "$backup_dir"
    [ -z "$release_id" ] || /usr/bin/rm -f -- "$OUTGOING/.${release_id}.tar.age.tmp" "$OUTGOING/${release_id}.tar.age"
  fi
  exit "$status"
}
trap cleanup EXIT
...
block_mutations() {
  compose run --rm --no-deps wpcli wp maintenance-mode activate --allow-root >/dev/null
  maintenance_armed=1
}
...
capture_release_state() {
  # Inventory is data, not executable release content: active and candidate
  # containers, image IDs, CMS versions, volume identities, and measured sizes.
  {
    compose ps --format json
    "$DOCKER" volume inspect wordpress_db_data wordpress_wp_data
    compose run --rm --no-deps wpcli wp core version
    compose run --rm --no-deps wpcli wp plugin list --format=json
    compose run --rm --no-deps wpcli wp post list --post_type=any --format=count
    /usr/bin/readlink -f "$CURRENT_ROOT"
    /usr/bin/du -sb -- "$backup_dir"
...
names = ('database.sql.gz', 'wordpress.tar.gz', 'release.tar.gz', 'nginx.conf', 'production.env', 'release-state.json')
files = {}
for name in names:
    path = root / name
...
try:
    ...
    if manifest.get('schemaVersion') != 'tio2-production-backup-v1' or not re.fullmatch(r'[0-9]{8}T[0-9]{6}Z-[a-f0-9]{40}', manifest.get('backupId', '')):
        raise ValueError
    files = manifest['files']
    if set(files) != {'database.sql.gz', 'wordpress.tar.gz', 'release.tar.gz', 'nginx.conf', 'production.env', 'release-state.json'}:
        raise ValueError
...
keep_newest_three() {
  ...
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    [[ "$name" =~ ^[0-9]{8}T[0-9]{6}Z-[a-f0-9]{40}$ ]] || fail 'backup directory identity is unsafe'
    if [ "$count" -lt 3 ]; then
...
wordpress_stopped=1
compose run --rm --no-deps wpcli tar -C /var/www/html -czf - . > "$backup_dir/wordpress.tar.gz"
...
/usr/bin/chown deploy:deploy "$ciphertext_temporary"
/usr/bin/chmod 640 "$ciphertext_temporary"
