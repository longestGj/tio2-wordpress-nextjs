...
export default async function RequestSampleRoute(){const {site,page}=await loadPage();const receiverReady=Boolean(process.env.NEXT_PUBLIC_TIO2_MY_WEB3FORMS_ACCESS_KEY?.trim());const jsonLd=serializeMalaysiaRequestSampleJsonLd(buildMalaysiaRequestSampleJsonLd(site));return <MalaysiaRequestSampleQueryPage page={page} receiverReady={receiverReady} structuredData={<script key="CONV-SAMPLE-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd}}/>}/>}
