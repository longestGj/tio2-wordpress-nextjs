...
export async function generateMetadata():Promise<Metadata>{const {site,page}=await loadPage();return buildMalaysiaRequestSampleMetadata(site,{indexingAuthorized:page.releaseControls.indexingAuthorized,seo:page.seo,env:process.env})}
export default async function RequestSampleRoute(){const {site,page}=await loadPage();const receiverReady=isWeb3FormsAccessKey(process.env.NEXT_PUBLIC_TIO2_MY_WEB3FORMS_ACCESS_KEY);const jsonLd=serializeMalaysiaRequestSampleJsonLd(buildMalaysiaRequestSampleJsonLd(site,page.seo));return <MalaysiaRequestSampleQueryPage page={page} receiverReady={receiverReady} structuredData={<script key="CONV-SAMPLE-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd}}/>}/>}
