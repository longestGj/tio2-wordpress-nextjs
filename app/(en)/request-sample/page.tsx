import {resolveMalaysiaSampleReceiverConfig} from '@/lib/request-sample/malaysia-sample-server'
...
export default async function RequestSampleRoute(){const {site,page}=await loadPage();const receiverReady=Boolean(resolveMalaysiaSampleReceiverConfig(process.env));const jsonLd=serializeMalaysiaRequestSampleJsonLd(buildMalaysiaRequestSampleJsonLd(site));return <MalaysiaRequestSampleQueryPage page={page} receiverReady={receiverReady} structuredData={<script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd}}/>}/>}
