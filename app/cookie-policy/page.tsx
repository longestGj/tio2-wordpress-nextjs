...
async function load() { const site = getCurrentSite(); if (site.id !== 'tio2-my') notFound(); return {site, page: await getMalaysiaLegalPage('LEGAL-COOKIE-EN')} }
...
export default async function CookiePolicyPage() { const {site, page} = await load(); const json = serializeMalaysiaLegalPageJsonLd(buildMalaysiaLegalPageJsonLd(site, page)); return <MalaysiaLegalPage page={page} structuredData={<script type="application/ld+json" dangerouslySetInnerHTML={{__html: json}} />} /> }
