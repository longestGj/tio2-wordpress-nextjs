...
for (const width of [1440, 768, 390] as const) test(`Document Guides ${width}px native links`, async ({page}) => {
  ...
  const module = page.locator('[data-module="document-guides"]')
  await expect(module.getByText('DOCUMENT GUIDES', {exact: true})).toBeVisible()
  await expect(module.getByRole('heading', {name: 'Document Guides'})).toBeVisible()
  await expect(module.getByText('Review practical guidance before requesting documents for a product grade.', {exact: true})).toBeVisible()
  const links = module.locator('a')
  await expect(links).toHaveCount(3)
  await expect(module.getByRole('link', {name: 'TDS, SDS and COA Guide', exact: true})).toHaveCount(1)
  await expect(module.getByRole('link', {name: 'REACH Documentation Guide', exact: true})).toHaveCount(1)
  await expect(module.getByRole('link', {name: 'Certificate of Origin Guide', exact: true})).toHaveCount(1)
  ...
  await module.screenshot({path: `.local-evidence/task6-documents/document-guides-module-${width}.png`, animations: 'disabled'})
})
