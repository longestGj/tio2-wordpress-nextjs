...
vi.mock('@/lib/wordpress/market-page-brazil-pt-v02-queries', () => ({getMalaysiaBrazilPtMarketPage: boundary.query}))
afterEach(() => { vi.clearAllMocks(); vi.resetModules() })

it.each(['tio2-a', 'tio2-b'] as const)('rejects %s before querying Brazil PT data', async id => {
  boundary.site.mockReturnValue(getSiteConfig(id))
  const route = await import('@/app/pt-br/markets/brazil/page')
...
it('propagates a missing Malaysia PT CMS record instead of rendering fallback copy', async () => {
  boundary.site.mockReturnValue(getSiteConfig('tio2-my'))
  const error = new Error('Controlled missing Brazil Portuguese record')
