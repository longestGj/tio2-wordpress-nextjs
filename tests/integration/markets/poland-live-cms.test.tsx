...
registerSharedWordPressMutationLock(Boolean(process.env.POLAND_LOCAL_PROBE_POST_ID))
...
it.skipIf(!process.env.POLAND_LOCAL_PROBE_POST_ID)('real local WordPress edit reaches GraphQL, DTO and server rendering, then restores',()=>{
  ...
  const output=execFileSync('docker',[...wordpressComposeArgs({...WORDPRESS_RUNTIME_MODE, runId: 'poland-live-cms'}),'run','--rm','--no-deps','--no-TTY','--user','33:33','-e','WP_ENVIRONMENT_TYPE=local','wpcli','wp','eval-file','/workspace/tests/infrastructure/php/poland-live-probe.php',postId,seedHash],{encoding:'utf8',timeout:45000})
  ...
  const root=`.local-evidence/poland-live-cms-${process.pid}-${Date.now()}`
