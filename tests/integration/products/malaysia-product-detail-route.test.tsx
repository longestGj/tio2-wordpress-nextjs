...
describe('M-350 route integration', () => {
  ...
  it('loads M-2196 through the same scoped route with seventeen source values and methods', async () => {
    routeMocks.getMalaysiaProductDetail.mockResolvedValue(
      toMalaysiaProductDetailDto(malaysiaM2196ProductDetailSource(), 'm-2196'),
    )
    const route = await import('@/app/products/[familySlug]/page')
    const markup = renderToStaticMarkup(await route.default(props('m-2196')))
    expect(routeMocks.getMalaysiaProductDetail).toHaveBeenCalledWith('m-2196')
    expect(markup).toContain('M-2196 Titanium Dioxide for Solvent-Based Furniture and Industrial Paint')
    expect(markup).toContain('data-label="Value">Sulfate</td>')
    expect(markup).toContain('data-label="Test method">ISO 591-1</td>')
    expect(markup).toContain('measurements are subject to the applicable CoA')
    expect(markup).toContain('"value":"Sulfate"')
