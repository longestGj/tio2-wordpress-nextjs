...
beforeEach(()=>{vi.stubEnv('TIO2_MY_REQUEST_SAMPLE_RECEIVER_URL','https://receiver.example.test/sample');vi.stubEnv('TIO2_MY_REQUEST_SAMPLE_RECEIVER_TOKEN','test-token');mocks.getCurrentSite.mockReturnValue(getSiteConfig('tio2-my'));mocks.getPage.mockResolvedValue(pageDto)})
...
describe('CONV-SAMPLE route',()=>{
  it('server-renders safe visible prefill, initial answers, one graph and shared Chrome',async()=>{
    ...
    expect(markup).toContain('data-site-scope="tio2-my"');expect(markup).toContain('Context brought from your previous page');expect(markup).toContain('value="M-2377" selected=""');expect(markup).toContain('United Kingdom');expect(markup.match(/application\/ld\+json/gu)).toHaveLength(1);expect(markup.match(/<header/gu)).toHaveLength(1);expect(markup.match(/<footer/gu)).toHaveLength(1);expect(markup).not.toContain('/contact');expect(markup).not.toContain('receiver.example.test');expect(markup).not.toContain('test-token')
    for(const item of pageDto.faq.items)expect(markup).toContain(item.answer)
  })
  it.each([{url:'',token:'test-token'},{url:'https://receiver.example.test/sample',token:''}])('server-renders no usable form or receiver secret when either readiness input is absent',async({url,token})=>{
    vi.stubEnv('TIO2_MY_REQUEST_SAMPLE_RECEIVER_URL',url);vi.stubEnv('TIO2_MY_REQUEST_SAMPLE_RECEIVER_TOKEN',token);const route=await import('@/app/request-sample/page');const markup=renderToStaticMarkup(await route.default({searchParams:Promise.resolve({})}))
    expect(markup).not.toContain('<form');expect(markup).not.toContain('id="sample-grade_id"');expect(markup).not.toContain('Submit Sample Request for Review');expect(markup).toContain('We cannot confirm sample requests right now.');expect(markup).toContain('The sample request form is not available. No request has been confirmed. Please try again later.');expect(markup).not.toContain('receiver.example.test');expect(markup).not.toContain('test-token')
  })
