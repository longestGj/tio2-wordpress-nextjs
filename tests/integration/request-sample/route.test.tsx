...
beforeEach(()=>{vi.stubEnv('NEXT_PUBLIC_TIO2_MY_WEB3FORMS_ACCESS_KEY','01234567-89ab-cdef-0123-456789abcdef');mocks.getCurrentSite.mockReturnValue(getSiteConfig('tio2-my'));mocks.getPage.mockResolvedValue(pageDto)})
...
describe('CONV-SAMPLE route',()=>{
  ...
  it('uses approved production indexing with query-independent metadata',async()=>{vi.stubEnv('VERCEL_ENV','production');vi.stubEnv('TIO2_MY_REQUEST_SAMPLE_INDEXING_RELEASE_AUTHORIZED','true');const route=await import('@/app/request-sample/page');await expect(route.generateMetadata()).resolves.toMatchObject({alternates:{canonical:'https://tio2malaysia.com/request-sample/'},robots:{index:true,follow:true}})})
})
