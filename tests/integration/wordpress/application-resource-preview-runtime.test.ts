...
interface Snapshot {
  ...
  errors: {incompleteStatus: number; unsafeStatus: number; forbiddenClaim: boolean; unsupportedMarkup: boolean; emptyTitle: boolean; duplicateRelation: boolean; nonDraftStatus: number; crossSiteStatus: number}
...
describe.runIf(runLiveWordPress)('live protected Site A Application/Resource preview', () => {
  ...
  it('rejects incomplete, unsafe, untitled, and duplicate-identity records and keeps non-draft/cross-site closed', () => {
    expect(snapshot.errors).toEqual({incompleteStatus: 422, unsafeStatus: 422, forbiddenClaim: true, unsupportedMarkup: true, emptyTitle: true, duplicateRelation: true, nonDraftStatus: 404, crossSiteStatus: 404})
  })
