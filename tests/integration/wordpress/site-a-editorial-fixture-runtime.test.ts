...
echo 'TIO2_EDITORIAL_TEST_SNAPSHOT '.wp_json_encode(['siteAIdentity'=>$identity,'siteAContentHash'=>'sha256:'.hash('sha256',(string)wp_json_encode($site_a_content)),'siteBHash'=>'sha256:'.hash('sha256',(string)wp_json_encode($site_b)),'rootIdentityHash'=>'sha256:'.hash('sha256',(string)wp_json_encode($roots)),'schemaVersion'=>(string)get_field('homepage_schema_version',$site_a_id,false),'counts'=>$counts]);
`
...
describe.runIf(runLiveWordPress)('Site A local editorial fixture updater', () => {
  it('rejects ineligible state without writes or preserves every non-content boundary when eligible', () => {
    const before = snapshot()

    const plan = powershell(['-PlanOnly'])
    if (before.siteAIdentity.status !== 'publish') {
    ...
    const failed = powershell(['-Apply', '-FailurePoint', 'after-fields'])
    expect(failed.status).not.toBe(0)
    expect(`${failed.stdout}\n${failed.stderr}`).toContain(
      'Site A editorial fixture transaction rolled back',
    )
    expect(snapshot()).toEqual(before)

    const applied = powershell(['-Apply'])
    expect(applied.status, `${applied.stdout}\n${applied.stderr}`).toBe(0)
    expect(applied.stdout).toContain('"mode":"apply"')
    const after = snapshot()

    expect(after.siteAIdentity).toEqual(before.siteAIdentity)
    expect(after.siteBHash).toBe(before.siteBHash)
    expect(after.rootIdentityHash).toBe(before.rootIdentityHash)
    expect(after.schemaVersion).toBe('homepage-v0.2-editorial-geo')
