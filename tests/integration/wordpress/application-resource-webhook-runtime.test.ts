...
interface Snapshot {
  ...
  denied: {siteB: boolean; mixed: boolean; incomplete: boolean; unsafeRelation: boolean; duplicateIdentityPath: boolean}
...
describe.runIf(runLiveWordPress)('live Site A Application/Resource webhook invalidation', () => {
  ...
  it('emits no editorial state for Site B, mixed scope, incomplete records, or unsafe relations', () => {
    expect(snapshot.denied).toEqual({siteB: true, mixed: true, incomplete: true, unsafeRelation: true, duplicateIdentityPath: true})
  })
