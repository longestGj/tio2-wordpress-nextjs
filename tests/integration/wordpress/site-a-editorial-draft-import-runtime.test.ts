...
function get_posts($arguments) { if (!empty($GLOBALS['assert_editorial_cross_type_lookup'])) { $expected = ['attachment','hidden_search_content','nav_menu_item','page','post','tio2_application','tio2_document','tio2_faq','tio2_grade','tio2_homepage','tio2_product','wp_block','wp_font_face','wp_font_family','wp_global_styles','wp_navigation','wp_template','wp_template_part']; if ($expected !== ($arguments['post_type'] ?? null)) throw new RuntimeException('Editorial identity/path lookup omitted a registered content-owning post type.'); } return 'public_path' === ($arguments['meta_key'] ?? null) ? ($GLOBALS['controlled_path_ids'] ?? []) : []; }
