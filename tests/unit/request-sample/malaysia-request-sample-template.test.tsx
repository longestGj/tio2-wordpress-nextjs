...
describe('CONV-SAMPLE page and form',()=>{
  ...
  it('sets aria-busy only while a submission is in flight',async()=>{
    ...
    expect(JSON.parse(String(calls[0]?.[1]?.body))).toMatchObject({grade_id:'M-2196',workflow_type:'sample'})
  })
  ...
  it('directly retries with retained values and same token, rotating only after a material edit',async()=>{
    ...
    const bodies=vi.mocked(fetch).mock.calls.map((call)=>JSON.parse(String(call[1]?.body)) as {request_token:string});expect(bodies[1]?.request_token).toBe(bodies[0]?.request_token);expect(bodies[2]?.request_token).not.toBe(bodies[0]?.request_token)
  })
