...
const options = {accessKey:'test-key',requestToken:'sample-token',sourceContext:{source_page_id:'PRODUCT-000'}}
...
describe('Sample same-origin receiver client',()=>{
 it('uses the shared browser transport and never the retained server endpoint',async()=>{
  ...
  const result=await submitMalaysiaSampleRequest(values,{...options,accessKey:'test-key',requestToken:'sample-token',fetcher})
 ...
 it('maps approved values and source context to the provider payload',async()=>{
  ...
  expect(JSON.parse(String(init.body))).toMatchObject({access_key:'test-key',site_scope:'tio2-my',page_id:'CONV-SAMPLE',workflow_type:'sample',locale:'en',request_token:'sample-token',source_page_id:'PRODUCT-000'})
