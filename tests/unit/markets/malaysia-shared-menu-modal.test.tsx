...
it('opens the shared dialog with Close focused, locks scroll, wraps both Tab directions and returns focus',()=>{
  ...
  const topRfq=dialog.querySelector('div > a[href="/request-a-quote/"]') as HTMLElement
  expect(topRfq).not.toBeNull()
  ...
  topRfq.focus();fireEvent.keyDown(topRfq,{key:'Tab',shiftKey:true});expect(document.activeElement).toBe(links[7])
  fireEvent.keyDown(links[7]!,{key:'Tab'});expect(document.activeElement).toBe(topRfq)
