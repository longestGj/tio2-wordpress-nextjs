...
describe('ordinary approved CMS writes against real WordPress and InnoDB', () => {
  it('rejects before mutation, commits exact approved content, rolls back failures and serializes competing processes', async () => {
    ...
    try {
      ...
      await run('setup')
      expect.soft(await run('ordinary-save')).toEqual({apiRejected:true,defaultRejected:true,restRejected:true,restSchemaRejected:true,unchanged:true,noEvents:true,noOpNoEvents:true,withdrawEvent:true,approvedMetaEvent:true,draftSaveAccepted:true})
      await run('setup')
      ...
      expect.soft(independent).toEqual({independentDuringHold:true,identityRacesBlocked:[true,true,true,true],targetAppendBlocked:true,appScopes:['tio2-my']})
