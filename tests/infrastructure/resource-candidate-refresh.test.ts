...
const seed='wordpress/seed/refresh-tio2-my-resource-candidate.php'
const prior=Object.fromEntries(['RES-R706','RES-CHEMOURS'].map(id=>[id,execFileSync('git',['show',`e803bf8:wordpress/plugins/tio2-site-model/config/tio2-my-editorial-${id.toLowerCase()}.json`],{encoding:'utf8'})]))
const oldReviews=JSON.parse(execFileSync('git',['show','e803bf8:wordpress/plugins/tio2-site-model/config/tio2-my-alternatives-review-evidence.json'],{encoding:'utf8'})).pages
function run(scenario:string){
 ...
 foreach(['RES-R706','RES-CHEMOURS'] as $i=>$page){$records[$i+1]=['page'=>$page,'scope'=>['tio2-my'],'contract'=>$input['prior'][$page],'review'=>$input['oldReviews'][$i]['currentReview']];}
 ...
 function tio2_editorial_review_manifest($id){return json_decode(file_get_contents('/workspace/wordpress/plugins/tio2-site-model/config/tio2-my-alternatives-review-evidence.json'),true);}
 ...
 if($scenario==='duplicate')$records[3]=$records[2];
 ...
 $before=$records;$args=['Apply'];putenv('D16_TIO2_MY_PRERELEASE_RESOURCE_REFRESH='.($scenario==='no-optin'?'0':'1'));
...
describe('bounded Resources successor refresh seed',()=>{
 it('upgrades only both exact historical payloads and their review binding',()=>{const r=run('upgrade');expect(r.error).toBeNull();expect(r.writes).toBe(4);expect(r.restored).toBe(false)})
 it.each(['scope','hash','review','duplicate','no-optin'])('preflights both records before any write: %s',scenario=>{const r=run(scenario);expect(r.error).not.toBeNull();expect(r.writes).toBe(0);expect(r.restored).toBe(true)})
 ...
 it('binds source hash and ordering before the retained create-only seed',()=>{
  ...
  expect(manifest[index+1].path).toBe('wordpress/seed/apply-tio2-my-editorial-five.php')
  expect(readFileSync('ops/prerelease/bootstrap-wordpress.sh','utf8')).toContain('D16_TIO2_MY_PRERELEASE_RESOURCE_REFRESH=1')
 })
})
