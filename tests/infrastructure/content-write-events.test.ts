  ...
  it('suppresses rollback/transaction HTTP, merges committed paths, persists failure and retries without writing', async () => {
    ...
    try {
      ...
      expect(committed.receipt).toMatchObject({siteId:'tio2-my',approvalId:'synthetic-events',committed:true,notificationState:'failed',
        changedPages:['APP-000','HOME-001']})
      expect(committed.receipt.receiptId).toMatch(/^[0-9a-f-]{36}$/)
      expect(committed).toMatchObject({duringQueueCount:1,otherQueuePreserved:true,writeCount:2,contentMatches:true})
      ...
      expect(await run('noactor',committed.receipt.receiptId)).toEqual({receipt:'event_permission',calls:[],contentUnchanged:true,writeCount:0})
      expect(await run('tamper',committed.receipt.receiptId)).toEqual({receipt:'event_receipt',calls:[],contentUnchanged:true,writeCount:0})
      ...
      expect(replay).toMatchObject({writeCount:0,contentUnchanged:true})
      ...
      expect(wrongAck).toMatchObject({writeCount:0,contentUnchanged:true})
