...
describe('local Site A Product draft import wrapper', () => {
  it('accepts only a literal local file, stages it, verifies the exact hash, and removes the stage', () => {
    ...
    expect(source).toContain('$SourceHashBefore')
    expect(source).toContain('$SourceHashAfter')
    expect(source).toContain('validate-product-manifest.mjs')
    expect(source).toContain('$RuntimeManifestPath')
    expect(source.indexOf('[System.IO.File]::Copy($ManifestPath, $RuntimeManifestPath, $false)'))
      .toBeLessThan(source.lastIndexOf('validate-product-manifest.mjs'))
...
describe('local Site A Product draft importer contract', () => {
  it('exposes the deterministic importer entry point', () => {
    ...
    expect(source).toContain("['tio2-a'] !== ($existing['scopes'] ?? null)")
    expect(source).toContain('Product ID collision is not scoped exactly to Site A')
    expect(source).toContain('SELECT p.*')
    expect(source).toContain('tt.taxonomy AS taxonomy')
    expect(source).not.toMatch(/https?:\/\//iu)
  })
})
