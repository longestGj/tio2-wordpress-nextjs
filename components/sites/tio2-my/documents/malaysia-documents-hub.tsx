...
export function MalaysiaDocumentsHub({documentsHub, structuredData}: {readonly documentsHub: MalaysiaDocumentsHubDto; readonly structuredData?: React.ReactNode}) {
  ...
  return <div className={styles.site} data-site-id="tio2-my" data-site-scope="tio2-my">
    ...
    <main className={styles.main}>
      ...
      <section className={`${styles.section} ${styles.guides}`} data-module="document-guides" aria-labelledby="guides-heading"><div className={styles.sectionIntro}><p className={styles.eyebrow}>{documentsHub.documentGuides.eyebrow}</p><h2 id="guides-heading">{documentsHub.documentGuides.heading}</h2><p>{documentsHub.documentGuides.intro}</p></div><ul>{documentsHub.documentGuides.items.map((item) => <li key={item.targetPageId}><a href={item.href}>{item.label}<span aria-hidden="true">→</span></a></li>)}</ul></section>
