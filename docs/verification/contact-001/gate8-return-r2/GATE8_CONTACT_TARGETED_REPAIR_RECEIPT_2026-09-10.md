...
The user subsequently confirmed `收到一封邮件，19：11分收到的` and then explicitly clarified `是浏览器提交的` for the approved receiver mailbox and the test identity `AUTHORIZED SYNTHETIC CONTACT TEST / No business inquiry`. This is recorded as `USER_CONFIRMED_MAILBOX_RECEIPT`: exactly one matching synthetic Contact message was observed at `2026-09-10 19:11 Asia/Shanghai` and is attributed by the user to Project Control's browser submission.

The mailbox display time `19:11` and Project Control's browser observation time `19:12:29` differ across systems. This is recorded as `TIMESTAMP_DISCREPANCY / USER_EXPLICIT_ATTRIBUTION_CONTROLS`; the timestamp difference must not override the user's explicit attribution or redirect the message to D16's 19:05 diagnostic POST. Project Control's browser submission now has a provider/UI positive observation and `USER_CONFIRMED` mailbox receipt.

For complete audit disclosure, D16 had already executed a separate authorized diagnostic at `2026-09-10T11:05:41.046Z`. Its payload safety gate passed and released one POST, but no HTTP response was observed within 45 seconds. It remains `SUBMISSION_NOT_CONFIRMED / NO_SEPARATE_MAILBOX_RECEIPT_CONFIRMED`; it is neither rewritten as failed nor delivered. It was not retried, and no further real submission was made. The observations remain separate and contain no Access Key, receiver address, test email or full payload.
