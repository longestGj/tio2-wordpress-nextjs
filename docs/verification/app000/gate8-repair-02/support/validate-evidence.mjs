...
const boundary = readJson('runtime/public-boundary-scan.json')
const text = (path) => readFileSync(new URL(path, root), 'utf8')
const boundaryEntries = (page) => [
  page.html, page.rsc, page.dom, page.head, page.schema, page.accessibility,
  page.inlineScripts, page.browserState, ...page.loadedClientScripts,
]
const clean = (entries) => entries.every((entry) => entry.status === 200 && entry.internalMarkerMatches.length === 0)

const checks = {
  implementationCommit: runtime.implementationCommit === '6ece488cf88f060457890ad7f37da0db752564ef',
  buildId: runtime.buildId === 'maNqNFg3Urit7VkLTu5KM',
  ...
  appBoundaryClean: clean(boundaryEntries(boundary.app)),
  rfqBoundaryClean: clean(boundaryEntries(boundary.rfq)),
  publicApiClean: boundary.publicApiInternalMarkerMatches.length === 0,
  ...
  targetedTests: /Tests\s+159 passed \(159\)/u.test(text('logs/targeted-tests.log')),
  playwright: /22 passed/u.test(text('logs/playwright.log')),
  typecheck: /Types generated successfully/u.test(text('logs/typecheck.log')),
  fullSuiteBaselineOnly: /Tests\s+1 failed \| 2768 passed \| 50 skipped/u.test(text('logs/full-unit.log')),
}
