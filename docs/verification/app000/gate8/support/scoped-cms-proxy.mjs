...
const port = Number(process.env.APP000_CMS_PORT ?? 4380)
const upstream = process.env.APP000_CMS_UPSTREAM ?? 'http://127.0.0.1:8080/graphql'
const applicationHubContract = JSON.parse(readFileSync(
  new URL('../../../../../wordpress/plugins/tio2-site-model/config/tio2-my-application-hub.json', import.meta.url),
  'utf8',
))
...
createServer((request, response) => {
  ...
  request.on('end', async () => {
    try {
      ...
      if (String(parsed.query).includes('malaysiaApplicationHubRecordJson')) {
        const record = {
          id: 'gate8-application-hub', modifiedGmt: '2026-09-08T08:00:00', status: 'publish',
          siteScopes: {nodes: [{slug: 'tio2-my'}]}, publishingFields: {publicPath: '/applications'},
          malaysiaApplicationHubContractJson: JSON.stringify(applicationHubContract),
          routeReadiness: Object.fromEntries(applicationHubContract.routeRegistry.map((route) => [
            route.targetPageId,
            !['APP-COAT', 'APP-PLAS', 'APP-MB', 'APP-INK', 'APP-PAPER'].includes(route.targetPageId),
          ])),
        }
        response.writeHead(200, {'content-type': 'application/json'})
        response.end(JSON.stringify({data: {malaysiaApplicationHubRecordJson: JSON.stringify(record)}}))
        return
      }
...
}).listen(port, '127.0.0.1', () => process.stdout.write(`app000-scoped-cms:${port}\n`))
