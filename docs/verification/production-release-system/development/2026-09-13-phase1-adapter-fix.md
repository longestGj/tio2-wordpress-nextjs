...
Server plan subsequently rejected the initial runner's combined journal/blocked-marker check. Migration success deliberately leaves historical blocked.json intact. Corrected the runner to require no active journal, and authenticate the completed journal against the successful receipt, exact ordered commits and canonical plan hash. Historical blocked.json remains preserved and hash-bound. Ten Linux upgrade tests passed, including historical marker acceptance and active/invalid completed journal rejection; bounded independent re-review found no blockers. This correction changes only the external administrator runner, not the already frozen 7f3597b4 administrator program bundle or the original frontend transaction.
