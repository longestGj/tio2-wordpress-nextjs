...
const base = process.env.TIO2_MY_BASE_URL ?? 'http://127.0.0.1:3230'
const output = resolve('docs/verification/tio2-my/trade4-app5-gate9-repair-20260908')
...
for (const source of uk.links) {
  const externalPage = await browser.newPage()
  try {
    const response = await externalPage.goto(source.href, {waitUntil: 'domcontentloaded', timeout: 45_000})
    officialSources.push({...source, status: response?.status() ?? null, finalUrl: externalPage.url(), title: await externalPage.title(), error: null})
  } catch (error) {
    officialSources.push({...source, status: null, finalUrl: externalPage.url(), title: await externalPage.title().catch(() => ''), error: error instanceof Error ? error.message : String(error)})
  } finally {
...
const result = {
  ...
  buildId: (await readFile('.next-gate9-repair-4a7e170/BUILD_ID', 'utf8')).trim(),
  frontend: base,
  cmsMode: 'read-only local GraphQL response overlay over isolated WordPress :8186; no CMS write',
