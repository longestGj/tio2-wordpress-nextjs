...
const out='docs/verification/tio2-my/country-editorial-integration-20260908';
fs.mkdirSync(out,{recursive:true});
const ids=['APP-COAT','APP-PLAS','APP-MB','RES-TRADE-EU','RES-TRADE-IN'];
const base='http://127.0.0.1:3226',build=fs.readFileSync('.next-country-editorial/BUILD_ID','utf8').trim();
...
try {
 for(const id of ids){
  ...
  await page.close();console.log(`${id}: 3226 approved target and actual click/back passed; 3029=${other.status}`);
 }
}finally{await browser.close();fs.writeFileSync(`${out}/results.json`,JSON.stringify({checkedAt:new Date().toISOString(),runtimeCode:process.env.INTEGRATION_CODE_COMMIT,baseDeliveryCommit:'a5230cd',sameCandidateClickReturn:true,countryLatestRepairsIncluded:true,gate9FindingClosure:false,rows:results},null,2)+'\n');}
