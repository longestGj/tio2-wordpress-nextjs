...
vi.mock('@/lib/wordpress/market-page-poland-v01-queries',()=>({getMalaysiaPolandMarketPage:boundary.query}))
...
it.each(['tio2-a','tio2-b'] as const)('rejects %s before querying Poland data, for body and metadata',async id=>{
  boundary.site.mockReturnValue(getSiteConfig(id))
  const route=await import('@/app/markets/poland/page')
