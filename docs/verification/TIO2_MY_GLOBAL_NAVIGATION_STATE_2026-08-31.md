...
## GRADE-M350 Product Detail activation addendum — 2026-09-01

- Following `PRODUCT-DETAIL-G7-PCR-02` closure and Gate 8 authorization, `/products/m-350/` directly consumes the same `MalaysiaGlobalHeader`, Mobile Menu, `MalaysiaGlobalFooter`, stylesheet, and Global Chrome V0.5 configuration. It passes only `currentPageId=PRODUCT-000` and `sourcePageId=GRADE-M350`; no Product Detail Header/Footer wrapper, copied configuration, or page-level Chrome selector was introduced.
- The shared infrastructure contract now scans the Product Detail component and stylesheet alongside Home, Markets, and Product Hub. It requires exactly one direct Header/Footer consumption and rejects selectors that could style the shared Header, Logo, navigation/current marker, RFQ, Mobile Menu, or Footer outside `<main>`.
- Product Detail runtime verification at 1440/1024/768/430/390/320 asserts production Header and Footer Logo decoding, bounding boxes and painted pixels; Product current state; visible `CURRENT=0`; Desktop 3px underline; Mobile 4px left marker; fixed Global RFQ; Footer content; keyboard behavior; Axe; and no horizontal overflow.
- The combined Home/Markets/Product Hub regression remains 14/14 PASS, while the Product Detail contract is 8/8 PASS. Evidence and hashes are recorded in `docs/verification/product-detail/GATE8_LOCAL_VERIFICATION_2026-09-01.md`. This remains local Gate 8 evidence only.
