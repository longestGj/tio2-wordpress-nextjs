...
归档日期：2026-09-07。以下逐字保留原README正文，仅用于追溯旧环境和命令，不是当前多网站工作入口。

当前规则见[AGENTS.md](../../AGENTS.md)，当前操作入口见[README.md](../../README.md)。下文的双站数量、首页版本、Node最低版本、验证范围和暂停状态均属于当时记录；执行命令前读取当前脚本、依赖和网站合同。不得据此执行正式迁移、全量验证、部署或其他未授权操作。
