...
- [ ] **Step 6: Implement fixed Stage/Recover/Status orchestration**
  ...
  `Stage` calls `New-ProductionPackage` using the sealed Gate A receipt, calls `build_adoption_archive.py` for one exact tool commit, creates a deterministic administrator tar plus SHA-256 manifest, and sends seven fixed files in one upload batch: the release archive and its manifest/proof, the administrator archive and its manifest, the age backup public key, and the production input. The release archive already contains the complete frontend, WordPress plugin, and all approved migration seeds; no later code, plugin, or content upload is required. It creates or reuses `.production/private/tio2-backup.agekey` through the fixed local recovery image, keeps the identity file local, and uploads only `backup.age.pub`. `Recover` downloads the exact phase-A receipt and ciphertext, performs hash verification and the isolated MariaDB/WordPress restore verifier, writes `adoption-evidence.json` by exclusive create, then uploads only that verification evidence. All run identity is pinned in `adoption-stage.json`.
