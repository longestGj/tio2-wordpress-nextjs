...
用户于2026-09-11确认将完整代码、插件和全部批准内容一次性送入服务器。现有少量默认或旧记录视为待替换的空环境内容。第一次 Apply 先完成服务器备份和异地恢复验证；第二次 Apply 使用发布包中逐文件哈希的生产迁移清单初始化全部批准内容，再验证56条已发布业务记录。已有内容恰好匹配批准指纹时只验证而不重复改写；不匹配时必须走完整初始化，不允许部分补写。
