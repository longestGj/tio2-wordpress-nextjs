...
接管保留现有 WordPress、容器 MariaDB 和两个数据卷，并增加 Next.js 前台双槽位。第一次 Apply 只建立可恢复备份；异地恢复验证通过后，第二次 Apply 从同一发布包一次性初始化全部批准业务内容并验证，再接入前台。任何内容或运行时验证失败都保持正式前台关闭并从本次备份恢复。
...
Plan 必须读取生产 CMS 的非秘密内容清单并生成现状指纹。至少包括网站身份、插件版本、已发布记录数量、页面/业务对象身份、状态、更新时间和内容哈希。

用户于2026-09-11确认将完整代码、插件和全部批准内容一次性送入服务器。现有少量默认或旧记录视为待替换的空环境内容。第一次 Apply 先完成服务器备份和异地恢复验证；第二次 Apply 使用发布包中逐文件哈希的生产迁移清单初始化全部批准内容，再验证57条已发布CMS业务记录（原56条加已批准Contact）。公开验收仍以56个业务页面、Thank You和404共58个对象为主清单，并额外验证已上线的Contact内部链接目标。已有内容恰好匹配批准指纹时只验证而不重复改写；不匹配时必须走完整初始化，不允许部分补写。
...
待发布源码必须是干净、准确、批准的 `main` 提交。任何旧提交的58对象或表单证据都不能授权新的生产包；合入 `main` 后必须重建预发布并重新封存 Gate A。
...
| 正式内容初始化或指纹验证 | 从本次备份恢复数据库和WordPress文件，保持前台未开放 |
