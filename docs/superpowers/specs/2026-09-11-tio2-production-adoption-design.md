...
- `apply <plan-hash>`：只接受 Plan 生成的完整哈希；它可恢复执行，但不能接收路径、命令、镜像、容器、网络或挂载等自由参数。
...
同一个 `apply <plan-hash>` 可以调用两次并从 root 日志恢复，但始终绑定同一计划。
...
4. 启用 HTTP-only 引导配置：ACME challenge 可访问，其他前台请求返回503。
5. 用 Certbot webroot 取得同时覆盖 apex 与 `www` 的证书并核对域名、证书链和有效期。
...
生产 CMS 与批准内容指纹一致后，固定程序验证实际容器、镜像、Build ID、站点身份、CMS身份、42条公开路径、全部内部链接、活动别名和 Nginx `8081` 响应。该阶段使用真实生产 CMS 和真实候选，不进行外部表单提交，公众仍访问不到候选。
...
5. 补齐 `127.0.0.1:8081` Nginx listener及真实代理验证。
