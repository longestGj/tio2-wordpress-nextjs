...
**状态：** 共享 CMS 模型已于 2026-09-12 获用户确认，等待书面设计复核
...
3. 每个前台网站独立准备、备份、部署、验证、回退和封存回执。
4. CMS 核心、插件、数据结构和自动数据迁移作为共享平台独立发布，并验证所有网站消费者。
5. Nginx 公共配置、共享 CMS 配置和前台网站专属配置具有明确归属。
6. 前台发布不读取、导出、停止或修改共享 CMS 数据库和其他网站的私有资源。
7. 未登记的 Nginx 配置、域名、端口、容器或文件继续失败关闭。
8. deploy 用户只调用固定主体和固定动作；路径、命令和运行时身份由 root 配置决定。
9. 将现有生产系统在不移动数据、不修改 Nginx 内容、不重载和不切流的情况下迁入新架构。
10. 保留当前候选、当前 `PREPARED` 状态和同一个本地 RunRoot，迁移成功后恢复原发布批次。
...
CMS 平台发布影响所有已登记网站。WordPress 核心、共享插件、GraphQL contract、内容结构、自动 seed 或数据迁移中的任何变化，都必须走 CMS 平台发布，先完整备份共享 CMS，再回归所有网站。
...
- 独立候选、活动版本、备份、状态和回退记录；
- 对共享 CMS 的接口版本依赖和本网站内容指纹。

前台发布只能读取共享 CMS，不能修改 WordPress、数据库、插件或内容。若候选需要执行 CMS 写入，它不再是单纯前台发布，必须拆分或升级为 CMS 平台发布。
...
目标是更新某个网站的 Next.js。允许修改该前台的代码、镜像、槽位、upstream 和公开证书；共享 CMS 只读。备份只覆盖该前台活动版本、专属配置、证书、状态和回退依据。
...
普通编辑人员通过 WordPress 修改内容不等同于程序发布，但必须使用 WordPress 修订、审计和 `site_scope` 权限。任何由发布程序执行的内容写入仍属于 CMS 发布。
...
- 活动前台基线；
- 服务器公共基线版本；
- CMS generation、contract 版本和目标作用域内容指纹；
- Build ID、备份、部署、公网验证和最终验收证据。

CMS 状态至少绑定：

- CMS generation 和共享代码版本；
- 数据结构及插件版本；
- 数据库、媒体和配置备份；
- 恢复验证；
- 所有登记前台的兼容性结果；
...
CMS generation 变化后，各前台保留自己的活动代码版本，但必须重新验证 CMS contract。未通过兼容性验证的网站不能使用新的 CMS 发布完成回执。
...
- 当前公开证书链与私钥；
- 前台登记、基线、活动身份和状态；
- 服务器公共基线和 CMS generation 引用。

它不包含共享数据库、WordPress、CMS 私钥或其他网站资源。前台回退只回切自己的镜像和 upstream，不回退 CMS。
...
6. 将 Next.js 活动版本、候选、前台 Nginx、upstream 和公开证书登记为 `site:tio2-my`；
7. 验证没有未知配置、混合主体文件、端口冲突、跨站 TLS 引用或错误 `site_scope` 回退；
8. 将首次接管时已完成真实恢复验证的 WordPress/数据库备份登记为 CMS 接管依据；
9. 写入迁移日志后，原子安装服务器、CMS 和前台登记；
10. 将现有 `PREPARED` 状态迁移为 `tio2-my` 前台状态，更新公共基线和 CMS generation 绑定，同时保持候选身份、候选目录和本地 RunRoot 不变；
11. 运行只读状态、CMS contract、目标作用域和前台备份前置检查；
12. 删除迁移日志并输出不含秘密的迁移回执。
...
2. 核对共享 CMS generation、`site_scope=tio2-my` 内容指纹和首次接管恢复验证依据；
3. 使用相同 RunRoot 重试固定 `Release`；
4. 生成并验证 `tio2-my` 前台备份，不停止或重复导出共享 CMS；
5. 完成候选内部构建和 Gate B；
6. 切流并完成服务器 `PUBLIC_VERIFIED`；
7. 执行 58 对象、174 浏览器案例的生产 E2E；
8. RFQ、Sample、Documents 各执行一次已授权的真实提交；
9. 用户按唯一令牌确认三封生产邮件；
10. 封存最终 `PRODUCTION_VERIFIED` 回执。
...
3. 前台 A 发布不停止、修改或归档 CMS，也不读取前台 B 私钥；
4. CMS 发布完整备份共享数据库，并触发 A、B 两个消费者回归；
5. 其他前台字节变化不使目标前台基线失效；
6. 公共配置漂移和未知 Nginx 文件在停止写入前失败；
7. Certbot 软链接合法续期通过，目录逃逸、错误 SAN、过期和密钥不配对失败；
8. 一个配置文件承载多个主体时接管失败；
9. 端口冲突、前台容器跨站复用和卷错误归属失败；
10. `PREPARED` 状态迁移保留候选身份，并绑定准确公共基线和 CMS generation；
11. 管理员安装和迁移在每个切换点中断后可恢复；
12. 前台备份归档不包含 CMS 数据、CMS 私钥或其他前台资源；
13. CMS 备份包含完整共享数据并通过真实解密/恢复验证；
14. 当前生产 Python 套件、基础设施测试和隔离 Linux 发布演练通过；
15. 管理员包由准确 Git commit 构建，两次构建字节一致并记录 SHA-256；
16. 生产迁移后先执行只读核验，再恢复同一 RunRoot。
...
9. deploy 用户完成首次前台备份、内部验证、切流和生产验收；
10. 封存该前台独立的生产回执。
