...
11. 先用已封存的 CMS 指纹判定当前候选是否是 `frontend-only`；一致时保留当前 `PREPARED` 和同一个 RunRoot，不一致时不得改写不可变包，须封存为 superseded 并重新生成 `combined` 网站包。
...
11. 比较生产 `site_scope=tio2-my` 指纹与当前预发布回执；一致时将现有 `PREPARED` 迁移为 `frontend-only` 网站状态，并保持候选身份、候选目录和本地 RunRoot 不变；
...
生产内容指纹与当前预发布回执一致时，迁移成功后继续使用本地 RunRoot：
...
3. 对比当前生产内容与预发布 CMS 指纹，要求本次准确归类为 `frontend-only`；如有差异，停止当前批次并在网站内容发布能力完成后生成新的 `combined` 包，不允许修改当前不可变 RunRoot；
...
先实现服务器、共享 CMS 和 `tio2-my` 前台的归属登记，修复 Nginx 与 TLS 基线，并比较生产 CMS 指纹和当前预发布指纹。如果两者一致，将当前批次迁移为 `frontend-only`，完成前台备份、Gate B、切流和生产验收。这一阶段不改造 WordPress 内容存储。
