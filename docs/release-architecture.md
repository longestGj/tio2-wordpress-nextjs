...
阶段一接管及兼容前台发布的独立种子清单与实际 CMS 内容比较见[CMS 比较说明](production-cms-comparison.md)。
...
设计或修改升级、合同迁移与恢复状态时，按[执行规则 R03–R07](release-engineering-rules.md#r03)验证生命周期、迁移完整性、实际消费者、分对象策略及跨程序证据兼容。该规则规定验收要求，不表示通用迁移或其他未安装能力已经可用。
