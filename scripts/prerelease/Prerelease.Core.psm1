...
function Complete-PrereleaseEvidence {
    ...
    $result = [ordered]@{
        ...
        requiredChecks = $(if ($Action -eq 'TestLiveForms') { @('direct_provider', 'thank_you', 'one_post_per_workflow', 'no_blocked_writes') } else { @('chromium', 'axe', 'keyboard', 'visible_focus', 'public_paths', 'internal_links_59', 'no_horizontal_overflow', 'back_return_state', 'five_application_provisional_metadata') })
        inventory = $(if ($Action -eq 'Test') { [ordered]@{registeredObjects=59;eligibleRoutes=58;cmsRoutes=57;nativeRoutes=1;homeActions=6;productGrades=14;productProcesses=2;productSupport=3;applicationChildren=5;applicationGradeOccurrences=30;applicationSupport=3;resourceItems=8;documentGuides=3} } else { $null })
