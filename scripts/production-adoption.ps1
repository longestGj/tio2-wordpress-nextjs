...
function Assert-AdoptionConnection($Config) {
    try {
        if ($Config.Keys.Count -ne 8 -or $Config.siteId -cne 'tio2-my' -or $Config.host -cne '129.146.68.82' -or $Config.port -ne 22 -or $Config.username -cne 'deploy') { throw 'invalid' }
...
function Send-AdoptionStage($Config, [string]$EvidenceRoot) {
    $allowed = @('release.tar.gz','release-manifest.json','release-proof.json','admin-bundle.tar.gz','admin-bundle-manifest.json','backup.age.pub')
...
function Invoke-AdoptionStage($Config) {
    ...
    return [pscustomobject][ordered]@{state='STAGED';runRoot=$root;commit=$package.commit;uploadedFiles=6;adminArchiveSha256=$admin.archiveSha256;backupPublicKeySha256=$identity.publicSha256;rootCommand=$rootCommand}
}
...
try {
    ...
    else { throw 'Status is not available until the fixed program is installed.' }
