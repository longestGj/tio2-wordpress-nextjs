...
const USAGE = 'Usage: node scripts/editorial/validate-site-a-resources.mjs [--allow-incomplete] <manifest.json>'
...
try {
  ...
  const {validateSiteAResourceManifest} = await loadEditorialTypeScriptModule(new URL('../../lib/resources/content-manifest.ts', import.meta.url))
  const manifest = validateSiteAResourceManifest(input, {allowIncomplete})
