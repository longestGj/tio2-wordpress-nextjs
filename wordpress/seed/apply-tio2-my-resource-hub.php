...
if (! function_exists('tio2_validate_resource_hub_v01_contract')) {
    throw new RuntimeException('Run this file through the project WordPress WP-CLI container.');
}

$site_id = 'tio2-my';
$internal_slug = 'tio2-my-resources';
$public_path = '/resources';
$contract_path = dirname(__DIR__) . '/plugins/tio2-site-model/config/tio2-my-resource-hub.json';
...
if (! is_string($contract_json) || '' === $contract_json || ! is_array(json_decode($contract_json, true))) {
    throw new RuntimeException('The approved Malaysia Resources Hub contract is missing or invalid.');
}
...
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local TiO2 Malaysia Resources Hub record.');
if ([] === $ids) {
    $created = wp_insert_post([
        'post_type' => 'tio2_resource_hub', 'post_status' => 'draft',
        'post_title' => 'TiO2 Malaysia Resources', 'post_name' => $internal_slug,
    ], true);
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The Malaysia Resources Hub contract could not be published locally.');
