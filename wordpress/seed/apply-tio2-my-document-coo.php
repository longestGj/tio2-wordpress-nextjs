...
if (! function_exists('tio2_validate_document_coo_v04_contract')) {
    throw new RuntimeException('Run this file through the project WordPress WP-CLI container.');
}

$site_id = 'tio2-my';
$internal_slug = 'tio2-my-document-coo';
$public_path = '/documents/certificate-of-origin';
$contract_json = tio2_my_document_coo_approved_contract_json();
...
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local DOC-COO record.');
$same_slug_ids = get_posts([
    'post_type' => 'tio2_doc_tds', 'post_status' => ['draft', 'pending', 'private', 'publish'],
    'name' => $internal_slug, 'fields' => 'ids', 'numberposts' => 2, 'suppress_filters' => false,
]);
if ([] === $ids && [] !== $same_slug_ids) throw new RuntimeException('A same-slug DOC-COO record exists outside site_scope=tio2-my.');
if ([] === $ids) {
    $created = wp_insert_post([
        'post_type' => 'tio2_doc_tds', 'post_status' => 'draft',
        'post_title' => 'TiO2 Malaysia Certificate of Origin Guide', 'post_name' => $internal_slug,
    ], true);
...
update_post_meta($post_id, TIO2_MY_DOCUMENT_COO_CONTRACT_META, $contract_json);
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The local DOC-COO record could not be published.');
echo wp_json_encode([
    'status' => 'passed', 'postId' => $post_id, 'siteScope' => $site_id,
    'internalSlug' => $internal_slug, 'publicPath' => $public_path,
    'schemaVersion' => 'document-coo-public-v0.4',
]) . PHP_EOL;
