...
if (! function_exists('tio2_validate_application_hub_v01_contract')) {
    throw new RuntimeException('Run this file through the project WordPress WP-CLI container.');
}

$site_id = 'tio2-my';
$internal_slug = 'tio2-my-applications';
$public_path = '/applications';
$contract_path = dirname(__DIR__) . '/plugins/tio2-site-model/config/tio2-my-application-hub.json';
...
if (! is_string($contract_json) || '' === $contract_json || ! is_array(json_decode($contract_json, true))) {
    throw new RuntimeException('The approved Malaysia Application Hub contract is missing or invalid.');
}
...
$ids = get_posts([
    'post_type' => 'tio2_application_hub', 'post_status' => ['draft', 'pending', 'private', 'publish'],
    'name' => $internal_slug, 'fields' => 'ids', 'numberposts' => 2, 'suppress_filters' => false,
]);
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local TiO2 Malaysia Application Hub record.');
if ([] === $ids) {
    $created = wp_insert_post([
        'post_type' => 'tio2_application_hub', 'post_status' => 'draft',
        'post_title' => 'TiO2 Malaysia Applications', 'post_name' => $internal_slug,
    ], true);
...
update_post_meta($post_id, TIO2_MY_APPLICATION_HUB_CONTRACT_META, $contract_json);
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The Malaysia Application Hub contract could not be published locally.');
echo wp_json_encode([
