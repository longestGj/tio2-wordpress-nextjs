...
$site_id = 'tio2-my';
$internal_slug = 'tio2-my-about';
$public_path = '/about';
$contract_path = dirname(__DIR__) . '/plugins/tio2-site-model/config/tio2-my-about-page.json';
...
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local TiO2 Malaysia About page record.');
if ([] === $ids) {
    $created = wp_insert_post([
        'post_type' => 'tio2_about_page', 'post_status' => 'draft',
        'post_title' => 'TiO2 Malaysia About', 'post_name' => $internal_slug,
    ], true);
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The Malaysia About page contract could not be published locally.');
echo wp_json_encode([
