...
$site_id = 'tio2-my';
$internal_slug = 'tio2-my-m-2377';
$public_path = '/products/m-2377';
...
$contract_json = $approved_grades['m-2377']['contract_json'] ?? false;
...
if (! is_string($contract_json) || '' === $contract_json || ! is_array($contract)) {
    throw new RuntimeException('The approved Malaysia M-2377 Product Detail contract is missing or invalid.');
}
...
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local TiO2 Malaysia M-2377 record.');
if ([] === $ids) {
    $created = wp_insert_post(['post_type' => 'tio2_grade', 'post_status' => 'draft', 'post_title' => 'M-2377 Product Detail — TiO2 Malaysia', 'post_name' => $internal_slug], true);
...
update_post_meta($post_id, TIO2_MY_ROUTE_PAGE_ID_META, 'GRADE-M2377');
update_post_meta($post_id, TIO2_MY_ROUTE_CANONICAL_META, 'https://tio2malaysia.com/products/m-2377/');
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The Malaysia M-2377 contract could not be published locally.');
