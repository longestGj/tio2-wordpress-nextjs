...
$site_id = 'tio2-my';
$internal_slug = 'tio2-my-m-886';
$public_path = '/products/m-886';
...
$contract_json = $approved_grades['m-886']['contract_json'] ?? false;
if (! is_string($contract_json) || '' === $contract_json || ! is_array(json_decode($contract_json, true))) {
    throw new RuntimeException('The approved Malaysia M-886 Product Detail contract is missing or invalid.');
}
...
if (count($ids) > 1) throw new RuntimeException('Expected no more than one local TiO2 Malaysia M-886 record.');
if ([] === $ids) {
    $created = wp_insert_post(['post_type' => 'tio2_grade', 'post_status' => 'draft', 'post_title' => 'M-886 Product Detail — TiO2 Malaysia', 'post_name' => $internal_slug], true);
...
update_post_meta($post_id, TIO2_MY_ROUTE_PAGE_ID_META, 'GRADE-M886');
update_post_meta($post_id, TIO2_MY_ROUTE_CANONICAL_META, 'https://tio2malaysia.com/products/m-886/');
...
if ('publish' !== get_post_status($post_id)) throw new RuntimeException('The Malaysia M-886 contract could not be published locally.');

echo wp_json_encode([
    'status' => 'passed', 'postId' => $post_id, 'siteScope' => $site_id,
    'internalSlug' => $internal_slug, 'publicPath' => $public_path,
    'recordState' => 'approved_for_preview', 'schemaVersion' => 'product-detail-v0.1-malaysia',
]) . PHP_EOL;
