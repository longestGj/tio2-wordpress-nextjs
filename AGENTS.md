...
- 理解或改动软件结构时先读 [当前软件架构](docs/software-architecture.md)，再按其中的代码入口核对；历史双站设计和局部方案按原范围解释，不单独作为当前全貌或部署证明。软件架构描述系统，开发流程描述执行方法。
- 按 [阅读导航](docs/development-workflow.md#0-按任务阅读与文档职责)选择适用文件：开发/测试读 [内部开发与测试执行](docs/development-execution.md)，CI/CD及两套本地环境读其中第10–13节，预发布操作读 [环境使用说明](docs/prerelease-environment.md)，发布读开发交付流程第7节。规则保存职责和约束，模板保存本次事实；不因文档齐全就宣称自动化、测试或部署已经完成。
...
- CMS、前端、配置、构建按[职责划分](docs/superpowers/specs/2026-09-13-cms-frontend-build-responsibilities-design.md)承接：CMS管理受控内容，前端按技术契约呈现，配置承载环境与功能策略，构建生成制品；不得新增运行读取与历史批准文案的相等绑定。业务批准与发布授权继续独立，既有耦合按授权分批整改，文档更新不代表已修复。
